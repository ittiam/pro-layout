<template>
  <a-form class="dynamic-form" :form="form" v-if="formData.list.length">
    <a-row :gutter="formData.layout.gutter">
      <a-col :span="formItem.layout.span" v-for="(formItem, index) in formData.list" :key="index">
        <a-form-item
        ...
        >
          <!-- 文本输入框 -->
          <a-input
            v-decorator="[formItem.fieldName, { initialValue: formItem.initialValue, rules: formItem.rules }]"
          ...
          >
            <a-select-option :value="opt.value" v-for="opt in formItem.options" :key="opt.value">
              {{ opt.label }}
            </a-select-option>
          </a-select>
          <!-- 树选择器 -->
          ...
          <a-switch
            v-decorator="[
              formItem.fieldName,
              { initialValue: formItem.initialValue, rules: formItem.rules, valuePropName: 'checked' }
            ]"
            :disabled="formItem.disabled"
            :fieldNames="formItem.fieldNames"
            v-bind="formItem.attrs"
          ...
          <template v-else-if="formItem.fieldType === 'upload'">
            <a-upload
              ...
              :disabled="formItem.disabled"
            >
              <div class="uploader-preview" v-if="formItem.previewImage">
              ...
              <div class="uploader-guide wfc2" v-else>
                <a-icon type="plus" class="wfc2" />
                <p>点击上传图片</p>
              </div>
              <div class="uploader-guide s-load" @click.stop v-if="formItem.loading">
                <a-icon type="loading" />
                <p>上传中...</p>
              </div>
            </a-upload>
            <p class="uploader-tip wfc3" v-if="formItem.tip">{{ formItem.tip }}</p>
          </template>
          <slot :name="formItem.slotName" v-else-if="formItem.fieldType === 'slot'"></slot>
        </a-form-item>
      </a-col>
    </a-row>
  </a-form>
  ...
  genFormData(formData) {
    ...
    if (formData && formData.list && formData.list.length) {
      ...
      formData.list.forEach((v, i) => {
        ...
        if (~['datePicker', 'monthPicker', 'rangePicker'].indexOf(v.fieldType)) {
          // 初始值格式化
          if (v.initialValue) {
          ...
          // 上传控件初始化预览图
          v.previewImage = _customData.imageFullUrl(v.initialValue);
          v.loading = false;
          ...
          for (let k in values) {
            ...
            if (item) {
              ...
              if (item.resultFormat) {
                ...
                } else if (Array.isArray(item.resultFormat) && item.resultFormat.every(v => typeof v === 'string')) {
